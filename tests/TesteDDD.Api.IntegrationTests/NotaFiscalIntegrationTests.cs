...
public class NotaFiscalIntegrationTests
{
    ...
    public async Task EmitirAsync_ComDadosValidos_DeveRetornarNotaAutorizada()
    {
        ...
        const string chaveAcesso = "35260401123456789012345678901234567890123456";
        ...
        var produto = new Produto("Teste Produto", 100, categoriaId, fornecedorId);
        ...
        notaFiscalEsperada.AtualizarChaveAcesso(chaveAcesso);
        notaFiscalEsperada.MarcarComoAutorizada("2026040100012345", "<xml>autorizado</xml>");

        _vendasRepositoryMock.Setup(r => r.GetByIdAsync(vendaId)).ReturnsAsync(venda);
        _clienteRepositoryMock.Setup(r => r.GetByIdAsync(clienteId)).ReturnsAsync(cliente);
        _notaFiscalRepositoryMock.Setup(r => r.ObterPorVendasIdAsync(vendaId)).ReturnsAsync((NotaFiscal?)null);

        _xmlGeracaoServiceMock
            .Setup(s => s.GerarXmlNFe(It.IsAny<NotaFiscal>(), It.IsAny<Vendas>(), It.IsAny<Cliente>(), It.IsAny<List<ItemVendas>>(), It.IsAny<string>()))
            .Returns("<xml>nfe</xml>");
        ...
        _notaFiscalRepositoryMock.Setup(r => r.AdicionarAsync(It.IsAny<NotaFiscal>())).Returns(Task.CompletedTask);
        ...
        Assert.Equal(3, resultado.Status);
        Assert.Equal(chaveAcesso, resultado.ChaveAcesso);
    ...
    public async Task EmitirAsync_ComVendaNaoExistente_DeveLancarExcecao()
    {
        ...
        _vendasRepositoryMock.Setup(r => r.GetByIdAsync(vendaId)).ReturnsAsync((Vendas?)null);
    ...
    public async Task ConsultarStatusAsync_ComChaveValida_DeveRetornarStatus()
    {
        ...
        notaFiscal.AtualizarChaveAcesso(chaveAcesso);
        notaFiscal.MarcarComoPendente();

        _notaFiscalRepositoryMock.Setup(r => r.ObterPorIdAsync(notaId)).ReturnsAsync(notaFiscal);
        _sefazServiceMock.Setup(s => s.ConsultarStatusAsync(chaveAcesso))
        ...
        _notaFiscalRepositoryMock.Setup(r => r.AtualizarAsync(It.IsAny<NotaFiscal>())).Returns(Task.CompletedTask);
    ...
    public async Task CancelarAsync_ComChaveValida_DeveRetornarNotaCancelada()
    {
        ...
        Assert.NotNull(resultado);
        Assert.Equal(5, resultado.Status);
    }
}
